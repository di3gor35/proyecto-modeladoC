...
export function GaussJordan() {
  ...
  const calculate = () => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      b[i] /= pivot
      newSteps.push(`F${i + 1} = F${i + 1} / ${pivot.toFixed(2)}`)
    ...
    newSteps.push("Matriz reducida a forma escalonada:")
    setResult(b)
    setSteps(newSteps)
    setError(null)
  }
  ...
  return (
    <div className="space-y-6">
      <Card className="p-6">
        <h2 className="text-2xl font-bold text-foreground">Método de Gauss-Jordan</h2>
        <p className="mt-2 text-muted-foreground">
          Reduce la matriz aumentada a forma escalonada reducida para obtener la solución directamente
        </p>
      </Card>
        ...
        <Card className="border-accent bg-accent/5 p-6">
          ...
          <div className="mt-6 rounded-lg bg-muted p-4">
            ...
            <p className="text-sm text-muted-foreground">
              Gauss-Jordan extiende la eliminación de Gauss para reducir la matriz a forma escalonada reducida (matriz
              identidad en la parte izquierda). Esto proporciona la solución directamente sin necesidad de sustitución
              hacia atrás.
            </p>
          </div>
        </Card>
