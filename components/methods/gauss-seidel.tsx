...
export function GaussSeidel() {
  ...
  const calculate = () => {
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
      ...
      const maxError = Math.max(...x.map((val, i) => Math.abs(val - xOld[i])))
      iterData.push({ iter: iter + 1, values: [...x], error: maxError })
  ...
  return (
    <div className="space-y-6">
      <Card className="p-6">
        <h2 className="text-2xl font-bold text-foreground">Método de Gauss-Seidel</h2>
        <p className="mt-2 text-muted-foreground">
          Método iterativo mejorado que usa valores actualizados inmediatamente
        </p>
      </Card>
        ...
        <Card className="border-accent bg-accent/5 p-6">
          ...
          <div className="mt-6 rounded-lg bg-muted p-4">
            ...
            <p className="text-sm text-muted-foreground">
              Gauss-Seidel mejora el método de Jacobi usando los valores recién calculados inmediatamente en la misma
              iteración. Esto generalmente resulta en convergencia más rápida. La fórmula es: x_i^(k+1) = (b_i - Σ a_ij
              * x_j^(k+1) - Σ a_ij * x_j^(k)) / a_ii
            </p>
          </div>
        </Card>
      )}
