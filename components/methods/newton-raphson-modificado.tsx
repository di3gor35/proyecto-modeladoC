...
export function NewtonRaphsonModificado() {
  ...
  const calculate = () => {
    try {
      ...
      // Calculate initial Jacobian (only once)
      const J0 = calculateJacobian(x)
      ...
      for (let iter = 0; iter < maxIterations; iter++) {
        ...
        const delta = solveLinearSystem(J0, negF)
  ...
  return (
    <div className="space-y-6">
      <Card className="p-6">
        <h2 className="text-2xl font-bold text-foreground">Newton-Raphson Modificado</h2>
        <p className="mt-2 text-muted-foreground">
          Variante del método de Newton que usa el Jacobiano inicial en todas las iteraciones
        </p>
      </Card>
        ...
        <Card className="border-accent bg-accent/5 p-6">
          ...
          <div className="mt-6 rounded-lg bg-muted p-4">
            ...
            <p className="text-sm text-muted-foreground">
              El método de Newton-Raphson Modificado usa el Jacobiano calculado en el punto inicial J(x₀) para todas las
              iteraciones: x^(k+1) = x^(k) - J(x₀)^(-1) × F(x^(k)). Esto reduce el costo computacional al evitar
              recalcular el Jacobiano, pero puede tener convergencia más lenta (lineal en lugar de cuadrática). Es útil
              cuando el cálculo del Jacobiano es costoso.
            </p>
          </div>
        </Card>
      )}
